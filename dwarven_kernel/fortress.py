...
Walls:
    ...
    Valteri  — Wall of Serenity
    Geddon   — Wall of Death

Each wall has states:
    (x^w)  set multiplying exponents/state
    (w^x)  inverse collapse state
    (xom)    null resonance state
...
vents at the bottom take in any processes or packets or shredded thing and gets fed to the worms for the star heart.
...
EVERY WALL has its own Gateway:
    ...
    If a wall fails the star heart creates a fire break were the vents are and burns everything for 20 seconds the wall is rebuilt in 18 seconds
...
Hearts/Quartermasters per wall:
    Each wall has its own two Heart/Quartermaster for energy distribution.
...
class WallState:
    ...
    (x^w)  set multiplying exponents
    (w^x)  inverse collapse state
    (xom)    null resonance state
    (f)    null consumption state
    """
    exponent_state: float = 1.0     # (x^w)
    collapse_state: float = 0.0     # (w^x) — 0 = stable, 1 = collapsed
    resonance_state: float = 1.0    # (xom) — 0 = null resonance
